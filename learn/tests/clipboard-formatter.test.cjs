...
describe('formatLessonForClipboard', () => {
  test('output starts with blockquote preamble', () => {
    const output = formatLessonForClipboard(testLesson, 2, 10);
  ...
  test('concept map included as "## Architecture Overview" with code fence', () => {
    ...
    assert.ok(output.includes('Tool Dispatch'), 'should contain concept map content');
    assert.ok(output.includes('YOU ARE HERE'), 'should contain YOU ARE HERE marker');
  });
  ...
  test('handles lesson without concept map', () => {
    ...
    const output = formatLessonForClipboard(lessonNoMap, 0, 5);
  ...
  test('handles lesson without project sections', () => {
    ...
    const output = formatLessonForClipboard(lessonNoProject, 0, 5);
